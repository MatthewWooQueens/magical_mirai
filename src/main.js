import { Application,Container,Text, Graphics, Point, Circle } from "pixi.js";
...
class letterObj{
  constructor(char,x,y){
    ...
    this.graphic.x = x
    this.graphic.y = Math.floor(Math.random() * (Math.floor(GAME_HEIGHT*0.7-font)-10) + 10)
  }
  move(delta){
    if(this.graphic.x > -8){
...
(async () =>{
  ...
  terrain.rect(0,0,GAME_WIDTH,GAME_HEIGHT*0.7).fill('#0e8ade')
  //width too small
  terrain.rect(0,GAME_HEIGHT*0.7,GAME_WIDTH,GAME_HEIGHT*0.3).fill('#008000')
  ...
  shoot.on('mousedown',(e)=>{
    ...
    anchorProjectile.circle(0,0,3).fill('#ffffff');
  ...
  app.ticker.add((delta)=>{
    if (!player.timer || !player.video) return;
    rect.clear()
    ...
    const progress = player.video.progress(position)
    ...
    if(down){
      ...
      const newHeight = Math.sqrt((direction.x)**2 + (direction.y)**2)**1.3
    ...
    for(const p of projectileList){
      if(p.x < -5 || p.x > GAME_WIDTH+5 || p.y < -5 || p.y>GAME_HEIGHT+5){
      ...
      for (const res of cellsToCheck) {
        ...
        for (const letters of spatialmap.spatialmap[res]) {
          console.log(spatialmap.spatialmap[res])
